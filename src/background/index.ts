...
import { backgroundDownloadImage } from "@ueu/ueu-canvas";
  ...
  searchForCourse: async (params: { queryString: string; subAccount: number }, _sender, sendResponse) => {
    ...
    try {
      ...
      sendResponse({
        success: false,
        error:
          e.message === "Cannot access a chrome:// URL"
...
runtime.onMessage.addListener((message: Record<string, any>, sender, sendResponse) => {
