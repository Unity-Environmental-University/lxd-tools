...
import getReferencesTemplate, { ReferenceExportType } from "@ueu/ueu-canvas";
...
describe("referencePageExistsValidation", () => {
  ...
  describe("run", () => {
    ...
    it("should return a failure test result when the references page is not found", async () => {
      ...
      expect(result.messages.reduce((agg, current) => [...agg, ...current.bodyLines], [] as string[])).toContain(
        "Learning Materials Page not found"
      );
    });
  });
