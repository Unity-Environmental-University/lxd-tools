...
import { rubricsForCourseGen, updateRubricAssociation } from "@ueu/ueu-canvas";
import { IIdHaver } from "@ueu/ueu-canvas";
import { updateAssignmentData } from "@ueu/ueu-canvas";
import { callAll } from "@ueu/ueu-canvas";
import AssignmentKind from "@ueu/ueu-canvas";
import { CourseValidation } from "@publish/fixesAndUpdates/validations/types";

import { IRubricAssociationData, IRubricData } from "@ueu/ueu-canvas";
...
export const rubricsTiedToGradesTest: CourseValidation<
  IIdHaver,
  RubricsTiedToGradeUserDataType | undefined,
  undefined
> = {
  ...
  run: async (course, config) => {
    try {
      ...
      const failureMessage = await Promise.all(
        callAll(
      ...
      const result = testResult(badAssociations.length <= 0, {
        failureMessage,
        userData: {
          badAssociations,
        },
...
export default [rubricsTiedToGradesTest];
