...
export const textSubEnabledBug: CourseValidation<
  Course,
  {
...
> = {
  ...
  description:
    "Ensure that discussions and quizzes are not adversely affected but the bug in 2.7.7 (added text_entry to discussions and quizzes).",
  async run(course) {
    try {
      ...
      const success =
        affectedAssignments.length + potentiallyAffectedDiscussions.length + potentiallyAffectedQuizzes.length === 0;
      ...
      const links = [...affectedAssignments, ...potentiallyAffectedDiscussions, ...potentiallyAffectedQuizzes].map(
        (l) => l.html_url
      );
  ...
  async fix(course, result) {
    ...
    const failureMessage = [...affectedQuizAssignments, ...affectedDiscussionAssignments].map((a) => ({
      links: [a.html_url],
