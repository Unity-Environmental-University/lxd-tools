...
describe("textSubEnabledBug Validation", () => {
  ...
  it("should correctly identify assignments with bad type information", async () => {
    ...
    const { _discussion } = assignmentfy({ discussion_topic: { ...localMockDiscussionData } } as IAssignmentData);
...
function _getMessages<UD = unknown>(testResultResult: ReturnType<typeof testResult<UD>>) {
