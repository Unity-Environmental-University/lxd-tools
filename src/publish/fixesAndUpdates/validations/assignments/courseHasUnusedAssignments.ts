...
import { IAssignmentData, SubmissionType } from "@ueu/ueu-canvas";
...
const run = async (course: UnusedAssignmentsCourse) => {
  ...
  for await (const assignment of assignments) {
    ...
    if (assignment.submission_types.includes("online_quiz")) checkFunc = (a) => quizIds.has(a.quiz_id);
      ...
      checkFunc = (a) => discussionIds.has(a.discussion_topic!.id);
...
export const courseHasUnusedAssignments: CourseValidation<
  UnusedAssignmentsCourse,
  {
