...
import {RunTestFunction} from "@publish/fixesAndUpdates/validations/types";

export const courseOverviewLanguageTest: CourseFixValidation<IContentHaver, Page> = {
...
// TODO; Hand off pertinent data as userData so fix can use it without having to refind it
type CourseOverviewLanguageUserData = {
    expectedHtml: string,
    page: Page,
    honorCodeDiv: HTMLDivElement,
}

async function run(course: IContentHaver): RunTestFunction<IContentHaver, Page> {
    //Find the course overview page
    const courseObj = await getCourseById(course.id);
    if(!courseObj.isUndergrad()) return;
    ...
    for(const div of divs) {
        if(div.textContent?.includes('By participating in this course, you agree:')) {
            honorCodeDiv = div;
            break;
        }
    }
    if(!honorCodeDiv) return testResult(false, {failureMessage: "Honor code div not found"});
    //Hand off a result
    return testResult(honorCodeDiv.innerHTML === expectedHtml, {
        ...
        //TODO; This isn't sending as CourseOverviewLanguageUserData
        userData: {
            expectedHtml,
            page,
            honorCodeDiv,
...
async function fix(course: IContentHaver, result?: ValidationResult<Page>) {
    //Get the test results from run
    const userData = this.run(course);
    if(!userData) return;
    //Attempt to fix
    const updateDiv = userData.honorCodeDiv.innerHTML;
    const updatePageLocal = userData.page.replace()

    const updatePage = await fetchJson(`/api/v1/courses/${course.id}/pages/${userData.page.page_id}`, {
        method: 'PUT',
        body: JSON.stringify({wiki_page: {}}),
    });
    //Hand off a response
    return testResult(updatePage.success, {
        userData,
        failureMessage: updatePage.success ? [] : [{bodyLines: ["Failed to update course overview"]}],
    });
}