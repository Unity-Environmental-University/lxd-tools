...
import { valueContainerCSS } from "react-select/dist/declarations/src/components/containers";
  ...
  const textContent = honorCodeDiv.textContent?.toLowerCase();

  console.log("textContent: ", textContent);

  if(!textContent) return testResult(false, { failureMessage: "Couldn't find text to compare." } );

  const success = keyPhrases.every(phrase => textContent.includes(phrase));
	...
	const { overviewPage, honorCodeDiv } = result.userData;
	//Attempt to fix
