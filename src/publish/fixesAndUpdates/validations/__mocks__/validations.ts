...
import { mockAssignmentData, mockDiscussionData, mockPageData, mockQuizData } from "@ueu/ueu-canvas";
import { ILatePolicyUpdate } from "@ueu/ueu-canvas";
import mockLatePolicy from "@ueu/ueu-canvas";

import { BaseContentItem } from "@ueu/ueu-canvas";
import { Quiz } from "@ueu/ueu-canvas";
import { Page } from "@ueu/ueu-canvas";
import { Discussion } from "@ueu/ueu-canvas";
...
export function badContentTextValidationTest(
  test: CourseValidation<IContentHaver>,
  badHtml: string,
  goodHtml: string,
  badContentItems?: IContentHaver[]
) {
  return async () => {
...
export function badContentTextValidationFixTest<ContentType extends BaseContentItem>(
  ...
  return async () => {
    ...
    const courseFunc = getCourses ?? contentGoofuses;
    {
      const goofuses = test.beforeAndAfters.reduce(
        (aggregator, [badExample, goodExample]) => [...aggregator, ...courseFunc(badExample, goodExample)],
        [] as IContentHaver[]
      );
      ...
      for (const goofus of goofuses) {
        ...
        if (!testResult.success)
          console.warn(
            testResult.messages.map((a) => a.bodyLines),
            goofus
          );
