...
export function testResult<UserData>(
  success: boolean | "unknown" | "not run" | undefined,
  options?: TestResultOptions<UserData>
...
export function matchHighlights(
  content: string,
  search: RegExp,
  maxHighlightLength = MAX_SEARCH_RETURN_SIZE,
  windowSize = SHOW_WINDOW
) {
  search.lastIndex = 0;
...
type CustomContentGetter<CourseType extends IContentHaver, ContentType extends BaseContentItem> = (
  course: CourseType
) => Promise<ContentType[]>;

export function badContentRunFunc<CourseType extends IContentHaver, ContentType extends BaseContentItem>(
  badTest: RegExp,
  ...
  return async (course: CourseType, config?: ICanvasCallConfig) => {
    ...
    const content = await (contentFunc
      ? contentFunc(course)
...
export function badSyllabusRunFunc(badTest: RegExp) {
...
export const isCorrectSectionFunc =
...
export type InSyllabusSectionFuncErrorData = ValidationErrorUserData<
  ISyllabusHaver,
  {
...
export function inSyllabusSectionFunc(
  ...
  if (typeof sectionHeaderSearch === "string" && sectionHeaderSearch.length == 0)
    throw new Error("Section header search strings must have a value. If you want to match ALL sections, use /.*/");
  if (typeof sectionBodySearch === "string" && sectionBodySearch.length == 0)
    throw new Error("Section body search strings must have a value. If you want to match all, use /.*/ instead");
...
export function addSyllabusSectionFix(
  ...
  return async (
    course: ISyllabusHaver & ICourseDataHaver,
    result: ValidationResult<InSyllabusSectionFuncUserData> | undefined
  ) => {
    ...
    if (Array.isArray(position)) {
      ...
      if (specialPosition === SpecialPosition.BeforeHeader) {
        ...
        if (!beforeHeaderEl) {
          return testResult(false, {
            failureMessage: `Could not find header to insert before ${headerSearch}`,
            userData: undefined,
          });
        }
...
export function badSyllabusStateFixFunc(
  ...
  // TODO; need to figure out how to get course here, using unknown as Course to clear error
  const course = "" as unknown as Course;
  const replaceText = replaceTextFunc(validateRegEx, replace);
  return async () => {
    try {
      ...
      await useSyllabusStore.getState().updateSyllabus(course, useSyllabusStore.getState().draftHtml);
      return testResult<never>(true);
    } catch (e) {
      return errorMessageResult(e);
    }
...
function replaceTextFunc(validateRegEx: RegExp, replace: string | ((text: string) => string)) {
  return (str: string) => {
    ...
    const out =
      typeof replace === "string" ? str.replaceAll(validateRegEx, replace) : str.replaceAll(validateRegEx, replace); //This is the silliest thing in the world, but the regex overloads wont recognize it otherwise.
...
export function errorMessageResult(e: unknown, links?: string[]) {
  const bodyLines = [e?.toString() || "Error"];
