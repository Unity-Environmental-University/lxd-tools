...
jest.mock("@ueu/ueu-cavas/dist/fetch/fetchJson");
describe("Bad Link Tests and Fixes", () => {
  describe("works for login proxies", () => {
    ...
    test(
      "Old Proxy Server link exists in course test works",
      badContentTextValidationTest(proxyServerLinkValidation, badProxyLinkPageHtml, goodProxyLinkPageHtml)
    );
    test("Old Proxy Server link replace fix works.", badContentTextValidationFixTest(proxyServerLinkValidation));
  });
