...
import { IGradingStandardData, IPagesHaver } from "@ueu/ueu-canvas/course/courseTypes";
...
describe("Grading policy validation correct test", () => {
  ...
  it("Fails gracefully if standards can't be found", async () => {
    const noPermissionsCourse = await mockModuleGradingPolicyHaver(null, [...mockGradModules], {
      overrides: {
        async getAvailableGradingStandards(_config?) {
          return [];
        },
        async getCurrentGradingStandard(_config?) {
          return null;
        },
      },
    });
    ...
    expect(
      result.messages.reduce(
        (aggregator, current, _index, _array) => aggregator + "\n" + current.bodyLines.join("\n"),
        ""
      )
