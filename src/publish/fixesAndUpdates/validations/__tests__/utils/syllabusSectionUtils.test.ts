...
describe("syllabus fix and validations functions", () => {
    ...
    let matchTest: (
        ...
        expectedResult?: Partial<ValidationResult<InSyllabusSectionFuncUserData>>) => () => Promise<void>;
    ...
    beforeEach(() => {
        ...
        matchTest = (sectionName, sectionText, expectedResult = {success: true}) => {
            ...
            return async () => expect(await inSyllabusSectionFunc(sectionName, sectionText)(mockCourse)).toEqual(expect.objectContaining(expectedResult))
        }
    });
    ...
    describe("section in syllabus", () => {
        ...
        test("Fails on regex with no match in section", async () => matchTest(/Section XYZ/, /line 1/i, { success: false })())
    })
    ...
    describe("addSyllabusSectionFix", () => {
        test('adds to end', async () => {
            ...
            expect(changeSpy).toHaveBeenCalledWith(normalizeIndent(`<div>
                <div>
                    <h3>Section Name</h3>
                    <p>Line 1</p>
                    <p>Line 2</p>
