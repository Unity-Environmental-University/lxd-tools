...
jest.spyOn(BaseContentItem.prototype, "saveData").mockImplementation(async (data) => {
  return data;
});
...
test("caps replace test", () => {
  ...
  expect(capitalize("moose MuncH")).toBe("Moose MuncH");
  expect(capitalize("moose MuncH")).toBe("Moose MuncH");
  let replacement = "Hello hello There".replace(/hello/gi, preserveCapsReplace(/hello/gi, "goodbye"));
  expect(replacement).toBe("Goodbye goodbye There");

  replacement = "HELLO HELLO THERE".replace(/hello/gi, preserveCapsReplace(/hello/gi, "goodbye"));
  expect(replacement).toBe("GOODBYE GOODBYE THERE");

  replacement = "Whoopsie".replace(/wh(oops)/gi, preserveCapsReplace(/wh(oops)/gi, "$1"));
...
describe("testResult", () => {
  ...
  it("correctly interprets a single string passed in for success and failure", () => {
    expect(testResult(false, { failureMessage: failure, links, notFailureMessage: success })).toEqual({
      ...validFailResult,
      messages: [{ bodyLines: [failure] }],
    });

    expect(testResult(true, { failureMessage: failure, links, notFailureMessage: success })).toEqual({
      ...validSuccessResult,
      messages: [{ bodyLines: [success] }],
    });
  });
  ...
  it("correctly interprets a list of strings passed in for success and failure", () => {
    expect(
      testResult(false, {
        failureMessage: failureMessageBodyLines,
        links,
        notFailureMessage: successMessageBodyLines,
      })
