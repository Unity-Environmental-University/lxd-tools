...
describe("discussionThreadingValidation", () => {
  ...
  test("run method should identify non-threaded discussions", async () => {
    ...
    const resultLines = [
      ...result.messages.reduce((previousValue, currentValue) => {
  ...
  test("fix method should convert non-threaded discussions to threaded", async () => {
    ...
    expect(DiscussionKind.put).toHaveBeenCalledWith(mockCourse.id, mockDiscussions[0].id, {
  ...
  test("run method should generate correct failure messages and links", async () => {
    ...
    (DiscussionKind.getHtmlUrl as jest.Mock).mockImplementation(
      (courseId, discussionId) => `url/${courseId}/${discussionId}`
    );
  ...
  test("fix method should handle failure to fix non-threaded discussions", async () => {
    ...
    (DiscussionKind.put as jest.Mock).mockImplementation(async (courseId, discussionId, _data) => ({
      id: discussionId,
      title: `Discussion ${discussionId}`,
