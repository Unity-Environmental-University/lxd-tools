...
import { getCourseDataGenerator } from "@ueu/ueu-canvas";
...
import { batchify, renderAsyncGen } from "@ueu/ueu-canvas";
...
export const dontUseThisValidation: CourseValidation = {
  name: "Dont use this",
  description:
    "If you are seeing this hallie either wanted you to and told you specifically, or Hallie has forgotten to comment out a debug setting." +
    " I promise you would remember if Hallie told you to use this. If you're " +
  ...
  async run(course) {
    ...
    for (const code of codes) {
      ...
      const courses = devCourses.filter(
        (a) => !(a.name.includes("DEPRECATED") || a.name.includes("OLD") || a.name.includes("MOVED"))
      ) as ICourseData[];
      // if(devCourse.name)
    ...
    if (failResults.length > 0 || failCourses.length > 0)
