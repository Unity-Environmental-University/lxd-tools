...
import { changeModuleLockDate, moduleGenerator } from "@ueu/ueu-canvas";
import { oldDateToPlainDate } from "@/date";

import { Course } from "@ueu/ueu-canvas";
import { assignmentDataGen, updateAssignmentDueDates } from "@ueu/ueu-canvas";
...
export function UpdateStartDate({
  ...
  async function changeStartDate() {
    ...
    try {
      ...
      affectedItems = [
        ...affectedItems,
        ...affectedAssignments.map((data) => new Assignment(data, course.id)).map(ContentAffectedRow),
      ];
