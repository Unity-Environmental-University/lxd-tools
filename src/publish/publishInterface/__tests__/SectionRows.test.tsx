...
import { mockUserData } from "@ueu/ueu-canvas/__mocks__";
import { Course } from "@ueu/ueu-canvas";
import { IProfile } from "@ueu/ueu-canvas/type";

jest.mock("../CourseRow", () => ({
  CourseRow: jest.fn(({ course, onSelectSection }) => (
    <div onClick={() => onSelectSection(course)}>PublishCourseRow: {course.id}</div>
  )),
}));

const mockCourses: Course[] = [{ id: 1, name: "Course 1" } as Course, { id: 2, name: "Course 2" } as Course];
