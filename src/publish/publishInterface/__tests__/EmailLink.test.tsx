import { getAdditionsTemplate, renderEmailTemplate } from "@/publish/publishInterface/EmailLink";
...
describe("Renders Email template", () => {
  ...
  test("render email with course specific info", async () => {
    jest.spyOn(Course.prototype, "getParentCourse").mockResolvedValue(undefined);
    ...
    jest.spyOn(PageKind, "getByString").mockResolvedValueOnce({ body: mockCourseSpecificContent } as any);
    ...
    expect(renderEmailTemplate(publishEmailMock, mockValues, [additionsTemplate])).toEqual(mockFilledSpecific);
  });
