...
import * as blueprintApi from "@ueu/ueu-canvas/course/blueprint";
import {
...
jest.mock("@ueu/ueu-canvas/course/getTermNameFromSections", () => ({
...
jest.mock("@ueu/ueu-canvas/course", () => ({
  Course: jest.requireActual("@ueu/ueu-canvas/course").Course,
  getCourseName: jest.requireActual("@ueu/ueu-canvas/course").getCourseName,
  getCourseData: jest.requireActual("@ueu/ueu-canvas/course").getCourseData,
  ...
  fetchJson: jest.fn().mockImplementation((url) => {
    ...
    if (url.includes("/syllabus")) {
      return Promise.resolve({
        syllabus_body:
          '<div class="cbt-callout-box"><p>Some content</p><p><strong>Term Name</strong></p><p><strong></strong></p></div>',
      });
    }
    ...
    if (url.includes("/courses/")) {
      return Promise.resolve({
        ...
        name: "Test Course",
        syllabus_body:
          '<div class="cbt-callout-box"><p>Some content</p><p><strong>Term Name: </strong>DE5W06.04.20</p><p><strong></strong></p></div>',
      });
    }
...
jest.mock("@ueu/ueu-canvas/course/migration", () => {
  const originalModule = jest.requireActual("@ueu/ueu-canvas/course/migration");
  return {
    __esModule: true,
    ...originalModule,
...
jest.mock("@ueu/ueu-canvas/course/migration/migrationCache", () => ({
  cacheCourseMigrations: jest.fn(),
  loadCachedCourseMigrations: jest.fn(() => []),
}));
jest.mock("@ueu/ueu-canvas/course/getSections", () => ({ getSections: jest.fn(() => mockSectionData) }));
    ...
    (blueprintApi.getBlueprintsFromCode as jest.Mock).mockResolvedValue([
      {
        ...mockBlueprintCourse,
        id: 101,
        getSyllabus: jest.fn().mockResolvedValue(mockSyllabus),
      },
    ]);
...
describe("Retirement and updates", () => {
  ...
  it("Creates a new course", async () => {
    ...
    assert(code);
    expect(createNewCourse).toHaveBeenCalledWith(
      code,
      mockBlueprintCourse.accountId,
      mockCourse.name.replace(mockCourse.courseCode!, code)
    );
  });
...
describe("Migrations", () => {
  ...
  it("waits for migration completion before continuing", async () => {
    ...
    jest.doMock("@ueu/ueu-canvas/fetch/fetchJson", () => ({
  ...
  it("checks assignment groups when migration completes", async () => {
    const getCourseById = jest.fn().mockResolvedValue({
      getAssignmentGroups: jest.fn().mockResolvedValue([{ name: "Assignments", group_weight: 0, id: 123 }]),
    });
    jest.doMock("@ueu/ueu-canvas/course", () => ({
      getCourseById,
    }));
  ...
  it("does not alert when no empty Assignments group exists", async () => {
    const getCourseById = jest.fn().mockResolvedValue({
      getAssignmentGroups: jest.fn().mockResolvedValue([{ name: "Assignments", group_weight: 5, id: 321 }]),
    });
    jest.doMock("@ueu/ueu-canvas/course", () => ({
      getCourseById,
    }));
