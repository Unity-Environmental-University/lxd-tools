...
const mockInstructorGuidePageUrl = 'sg-instructor-guide';
...
const aiSourceModules: IModuleData[] = [
    { ...mockModuleData, id: mockAIModuleId, name: 'Skills Gap', published: true, items: [] },
    {
        ...mockModuleData,
        id: 2,
        ...
        published: true,
        items: [
            { id: 10, module_id: 333, position: 1, indent: 0, content_id: 10, title: 'SG Guide Page', html_url: 'sg-instructor-guide', new_tab: true, type: 'Page', page_url: mockInstructorGuidePageUrl }
        ]
    }
];
...
describe('skillsGapModule Setup', () => {
    ...
    it('should stop and alert if Skils Gap module already exists in BP', async () => {
        mockCourse.getModules.mockResolvedValueOnce([
            ...bpInitialModules,
            { id: 2, name: 'Skills Gap', published: true, items: [] }
        ]);

        await skillsGapModuleSetup(props);

        expect(window.alert).toHaveBeenCalledWith("Skills Gap module already exists in BP.");
