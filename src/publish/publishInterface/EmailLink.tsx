...
export const EmailLink = React.memo(function EmailLink({
  ...
  const subject = useMemo(
    () => encodeURIComponent(course.name?.replace("BP_", "") + " Section(s) Ready Notification"),
    [course.name]
  );
  ...
  const copyToClipboard = useCallback(async () => {
    ...
    if (additionsTemplate) {
      additionsTemplates.push(additionsTemplate);
    } else {
      if (course.baseCode) {
    ...
    const body = renderEmailTemplate(
      emailTemplate,
      {
        userName: localStorage.getItem("USER_NAME") || user.name,
        userTitle: localStorage.getItem("USER_TITLE") || "Learning Experience Designer",
    ...
    try {
      ...
      console.error(e);
      setErrorMessages((prev) => [...prev, "Unable to copy to clipboard. Please try again."]);
    }
    console.log(body);
