...
export async function processKalturaVideos(_migrationId: string): Promise<MigrationVideo[]> {
  // Stub: Replace with actual logic to fetch modified links from Kaltura
  return []; // Return an array of modified links
}

export const preFinalizeMigration = createAsyncThunk<
  void,
  { id: string },
  {
    state: RootState;
...
>("kaltura/preFinalizeMigration", async ({ id }, { dispatch, getState }) => {
