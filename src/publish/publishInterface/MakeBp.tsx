...
export function MakeBp({ devCourse, onBpSet, onTermNameSet, onSectionsSet }: IMakeBpProps) {
  ...
  useEffect(() => {
    const isDisabled =
      isLoading || !!currentBp || !devCourse.parsedCourseCode || devCourse.parsedCourseCode.length == 0;
  ...
  async function updateBpInfo(course: Course, code: string) {
    const [bp] = (await getBlueprintsFromCode(code, [course.rawData.account_id, course.rawData.root_account_id])) ?? [];
    ...
    if (termDate) {
      ...
      if (daysLeft <= 5) {
        const confirmFinish = confirm(
          `Term ${termName} appears to still be in the future. Are you SURE you want to archive?`
        );
      ...
      try {
        ...
        for (const group of bpAssignmentGroups) {
          if (group.name === "Assignments" && group.group_weight === 0) {
            const deleteGroup = await fetchJson(`/api/v1/courses/${bpCourse.id}/assignment_groups/${group.id}`, {
              fetchInit: {
                method: "DELETE",
                body: formDataify({}),
              },
  ...
  return (
    <div>
      {!isDev && (
        <Row>
          <Col className={"alert alert-warning"}>This is not a DEV course</Col>
        </Row>
      )}
      {currentBp && (
        <Row>
          <Col sm={3}>
            <Button id={"archiveButton"} onClick={onArchive} disabled={isArchiveDisabled}>
              Archive {currentBp.parsedCourseCode}
            </Button>
          </Col>
          <Col sm={2}>
          ...
          <Col>{!termName && <Alert>Term name not found in BP sections.</Alert>}</Col>
        </Row>
      )}
      <hr />
      {
        <>
          <Row>
            <Col sm={3}>
              <Button id={"newBpButton"} onClick={onCloneIntoBp} aria-label={"New BP"} disabled={isNewBpDisabled}>
                Create New BP
              </Button>
            </Col>
            <Col sm={3}>
            ...
            <Col sm={5}>
              {currentBp &&
                activeMigrations.map((migration) => (
                  <DevToBpMigrationBar migration={migration} onFinishMigration={finishMigration} course={currentBp} />
                ))}
            </Col>
          </Row>
        </>
