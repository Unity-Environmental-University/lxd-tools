...
export const selectCourses = createSelector([selectCoursesById], (coursesById) => Object.values(coursesById));
...
export const selectCourse = (id: number) => {
  if (!courseSelectorsCache.has(id)) {
    courseSelectorsCache.set(
      id,
      createSelector([selectCoursesById], (coursesById) => coursesById[id])
    );
  }
...
export const selectCourseLoadingStatus = (id: number) => {
  if (!courseStatusSelectorsCache.has(id)) {
    courseStatusSelectorsCache.set(
      id,
      createSelector([selectCoursesStatusById], (statusById) => statusById[id])
    );
  }
...
export const selectCoursesByTermIds = (termIds: number[]) =>
  createSelector([selectCourseIdsByTermId, selectCoursesById], (idLookup, coursesById) =>
    termIds.flatMap((termId) => Array.from(idLookup[termId] || []).map((courseId) => coursesById[courseId]))
  );
