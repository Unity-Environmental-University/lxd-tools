...
export function criteriaAssessmentRows(
  rubricAssessment: RubricAssessment | null | undefined,
  criteriaInfo: CriteriaInfo | null,
  submissionBaseRow: Array<string | number | null | undefined>
) {
...
export function getCritIdsAndAssessments(
  rubricAssessment: RubricAssessment | undefined | null,
  criteriaInfo?: CriteriaInfo | null
) {
...
export function getSubmissionBaseRow({
  ...
  const { assignment } = submission;
  const { weekNumber, moduleName, numberInModule, type } = getModuleInfo(assignment, modules, assignmentsCollection);
...
export function submissionHeader(
  submissionBaseRow: Array<any>,
  submission: any,
  assignment: IAssignmentData,
  rubricId: string
) {
  return [...submissionBaseRow, rubricId, "Total", "Total", submission.grade, assignment.points_possible] as Array<
...
export function parseSubmissions(
  user: IUserData,
  userSubmissions: IAssignmentSubmission[] | { user_id: number; submissions: IAssignmentSubmission[] }[]
) {
