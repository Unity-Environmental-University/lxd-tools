...
import { IUserData, IEnrollmentData, IModuleData } from "@ueu/ueu-canvas";
...
import mockAssignmentSubmission from "@ueu/ueu-canvas";
...
describe("getSubmissionBaseRow", () => {
  it("should return correct base row for a submission", () => {
    ...
    const enrollment: IEnrollmentData = {
      user: { ...mockUserData, name: "John Doe", sis_user_id: "123" },
...
describe("submissionHeader", () => {
  it("should return correct submission header", () => {
    ...
    expect(result).toEqual(["base", "row", "rubric1", "Total", "Total", "A", 10]);
  });
});
...
describe("fillEmptyCriteria", () => {
  it("should fill empty criteria in critAssessments based on critOrder", () => {
    const critAssessments: CriteriaAssessment[] = [{ id: "a", points: 5, rating: "Average" }];
...
describe("criteriaAssessmentRows", () => {
  ...
  it("should create rows for criteria assessments", () => {
    ...
    expect(result).toEqual([
      [...mockSubmissionBaseRow, "crit1", 1, "Criteria 1", 10, 10],
      [...mockSubmissionBaseRow, "crit2", 2, "Criteria 2", 15, 20],
    ]);
  });
  ...
  it("should handle missing criteria info", () => {
    ...
    expect(result).toEqual([
      [...mockSubmissionBaseRow, "crit1", 1, "-REMOVED-", 10, undefined],
      [...mockSubmissionBaseRow, "crit2", 2, "-REMOVED-", 15, undefined],
    ]);
  });
  ...
  it("should handle missing rubric assessment", () => {
    ...
    expect(result.map((a) => [a[15], a[16]])).toEqual([
      ["Criteria 1", null],
      ["Criteria 2", null],
    ]);
  });
