...
export async function getRowsForSections(
  sections: SectionData[] | ICourseData[],
  sectionsAtATime = MAX_SECTION_SLICE_SIZE
) {
  ...
  while (sectionsLeftToProcess.length > 0) {
    ...
    const rowsOfRows = await Promise.all(
      sliceToProcessNow.map(async (section) => {
