...
export async function exportSectionsInTerm(
  course: (ICourseData & { term: ITermData }) | null = null,
  term: Term | number | null = null
) {
  const courseId = course ? course.id : getCourseIdFromUrl(document.documentURI);
  if (!course) {
    if (!courseId) return;
    course = (await getCourseData(courseId, { queryParams: { include: ["term"] } })) as ICourseData & {
      term: ITermData;
    };
  }
  assert(course);
