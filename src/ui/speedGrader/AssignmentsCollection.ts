...
import { CanvasData, IModuleItemData, ModuleItemType } from "@ueu/ueu-canvas";
import { IAssignmentData, IDiscussionData } from "@ueu/ueu-canvas";
...
export class AssignmentsCollection {
  ...
  constructor(assignments: IAssignmentData[]) {
    ...
    this.discussions = assignments
      .filter(
        (assignment) => assignment?.discussion_topic && DiscussionKind.dataIsThisKind(assignment.discussion_topic)
      )
  ...
  getContentById(id: number): IAssignmentData | undefined {
    for (const collection of [this.assignmentsByQuizId, this.assignmentsByDiscussionId, this.assignmentsById]) {
