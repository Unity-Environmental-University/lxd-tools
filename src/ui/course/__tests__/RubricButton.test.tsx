...
jest.mock("@ueu/ueu-canvas/dist/course");
jest.mock("@ueu/ueu-canvas/dist/content/assignments/legacy");
jest.mock("@ueu/ueu-canvas/dist/content/assignments");
jest.mock("@ueu/ueu-canvas/dist/fetch/fetchJson");
jest.mock("@ueu/ueu-canvas/dist/canvasUtils", () => ({
...
describe("RubricButton", () => {
  ...
  describe("Assignment rubric update", () => {
    ...
    it("should successfully update existing rubric", async () => {
      (getRubric as jest.Mock).mockResolvedValueOnce(mockRubric).mockResolvedValueOnce(mockRelatedRubric);
  ...
  describe("Discussion rubric update", () => {
    ...
    it("should throw error for ungraded discussion", async () => {
      ...
      await waitFor(() => {
        expect(mockAlert).toHaveBeenCalledWith(expect.stringContaining("Discussion does not have an assignment id."));
      });
    });
  });
  ...
  describe("Error handling", () => {
    it("should handle missing related course", async () => {
      ...
      await waitFor(() => {
        expect(mockAlert).toHaveBeenCalledWith(expect.stringContaining("Related course not found"));
      });
    });
    ...
    it("should handle course that is neither dev nor blueprint", async () => {
      ...
      await waitFor(() => {
        expect(mockAlert).toHaveBeenCalledWith(expect.stringContaining("Course is not a blueprint or dev course"));
      });
    });
    ...
    it("should handle unsupported content type", async () => {
      ...
      await waitFor(() => {
        expect(mockAlert).toHaveBeenCalledWith(expect.stringContaining("Content is not an assignment or discussion"));
      });
    });
  });
