...
export function BpButton({ course, currentBp }: BpButtonProps) {
  ...
  async function openMainBp(_e: FormEvent) {
  ...
  return (
    <>
      <Col>
        <Button title={bpBtnTitle} onClick={openMainBp} disabled={currentBp?.id === course.id || isBpDisabled}>
          BP
        </Button>
        <Button onClick={(_e) => setOpen(true)} title={bpsBtnTitle} disabled={isBpsDisabled}>
          BPs
        </Button>
        <Modal isOpen={open} requestClose={() => setOpen(false)}>
          {bps.toSorted(bMinusASortFn((a) => a.id)).map((bp, i) => (
            <Row key={`${bp.id}-${i}`}>
              <Button onClick={(_e) => openThisContentInTarget(course, bp.id)}>{bp.course_code}</Button>
            </Row>
          ))}
        </Modal>
      </Col>
