...
export function RubricButton({ course }: RubricButtonProps) {
  ...
  async function insertRubric(course: Course) {
    setIsLoading(true);
    if (
      !confirm("This will try to update the rubric for the assignment based on the same assignment in DEV/BP. Confirm?")
    ) {
      setIsLoading(false);
      return;
    }
      ...
      if (assignmentRubric && rubricAssociation) {
        ...
        (relatedRubric.data ?? []).forEach((criterion: any, i: number) => {
          ...
          (criterion.ratings ?? []).forEach((rating: any, j: number) => {
            ...
            updatedRubric[`rubric[criteria][${i}][ratings][${j}][long_description]`] =
              cleanText(rating.long_description) ?? "";
        ...
        (relatedRubric.data ?? []).forEach((criterion: any, i: number) => {
          ...
          (criterion.ratings ?? []).forEach((rating: any, j: number) => {
            ...
            newRubric[`rubric[criteria][${i}][ratings][${j}][long_description]`] =
              cleanText(rating.long_description) ?? "";
