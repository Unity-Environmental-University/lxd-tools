...
export function SelectValidations({
  ...
  const [_allValidationOptions, setAllValidationOptions] = useState(optionize(allValidations));
  function _getTestName(test: CourseValidation) {
    return test.name;
  }
        ...
        <Row>
          <Col sm={12}>
            <MultiSelect
              ...
              onSelectionChange={setValidationsToRun}
            ></MultiSelect>
            <button onClick={runValidations}>Run Tests</button>
          </Col>
          {/*Clear button doesn't currently work*/}
          <Col>
            <button onClick={() => setCoursesToRunOn([])}>Clear</button>
          </Col>
        </Row>
      </Form>
      <Row>
        <Col sm={12}>
          <CustomSearchValidation
            onGenerateSearchValidation={async (validation) => {
