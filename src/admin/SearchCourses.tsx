...
export function SearchCourses({ setFoundCourses, onlySearchBlueprints, includeLegacyBps }: ISearchCoursesProps) {
  ...
  const search: FormEventHandler = async (e) => {
    ...
    for (const code of seekCourseCodes) {
      ...
      for await (const course of generator) {
        ...
        courses = [...courses, optionCourse].toSorted((a, b) =>
          (a.baseCode ?? a.name).localeCompare(b.baseCode ?? b.name)
        );
  ...
  return (
    <Form onSubmit={search}>
      <input type={"text"} value={courseSearchString} onChange={(e) => setCourseSearchString(e.target.value)}></input>
