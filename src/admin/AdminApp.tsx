...
import { batchify, filterUniqueFunc } from "@ueu/ueu-canvas";
...
import { beginBpSync } from "@ueu/ueu-canvas";
...
export function AdminApp({ course, allValidations }: IAdminAppProps) {
  ...
  const [validationResultsLut, dispatchValidationResultsLut] = useReducer(
    listLutDispatcher<number, IIncludesTestAndCourseId>,
    {}
  );
  ...
  function cacheAssociatedCourses(bpId: number, toAdd: Course[] | Course) {
    ...
    console.log(
      bpId,
      sections.map((a) => a.parsedCourseCode)
    );
            ...
            {validationResults
              ...
              .map((result) => {
                ...
                return (
                  course && (
                    <Row>
                      <Col sm={4}>{course?.courseCode}</Col>
                      <Col sm={4}>{course.courseCode}</Col>
                      <Col sm={4}>
  ...
  function FoundCoursesDisplay() {
    return (
      <>
        <button onClick={() => setCoursesToRunOn(Array.from(new Set([...coursesToRunOn, ...foundCourses])))}>
          Add All
        </button>
        ...
        {foundCourses.map((course) => (
          <Row key={course.id}>
            <Col>{course.label ?? course.parsedCourseCode ?? course.name}</Col>
            <Col>
              <button onClick={() => setCoursesToRunOn([...coursesToRunOn, course])}>{"ADD"}</button>
            </Col>
          </Row>
        ))}
      </>
      ...
      <Row>
        <Col>
          <h3 style={{ fontSize: slim ? "0.5em" : "1em" }}>
            <a href={course.htmlContentUrl} target={"_blank"}>
              {course.parsedCourseCode ?? course.name}
            </a>
