...
export function getBannerImage(overviewPage: BaseContentItem) {
  ...
  if (pageBody.querySelectorAll<HTMLImageElement>(".cbt-banner-image img").length > 1) {
    alert(
      `The banner in ${overviewPage.name} currently has more than one banner image. ` +
        `If you can only see one image, there may be a hidden duplicate in the HTML. ` +
        `Use the HTML editor to find and remove it, then rerun the hometile process.`
    );
    return undefined;
  } else {
